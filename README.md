...
```QID\tDOCID\t1```
Modify the ```run_file``` variable in ```create_train_pkl_file.py``` to point to your desired ```run_file```.
...
## BERT-QPP<sub>cross</sub>
 ...
 4. the results will be saved in ```results``` directory in the following format:
